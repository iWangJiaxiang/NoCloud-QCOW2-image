...
- **安全防冲突设计**:
  ...
  - 支持根分区和文件系统**开机自动扩容**，底层无缝适配 PVE/QEMU 等环境的动态磁盘扩展。
  - 构建结束自动执行缓存垃圾清理。
...
本镜像内置了 `cloud-initramfs-growroot` 与 `systemd-growfs`，支持**全自动硬盘扩容**。

**自动扩容（推荐）**：
如果您在虚拟机管理界面（如 PVE）中增加了虚拟磁盘的大小，或者使用 `qemu-img resize` 扩大了镜像体积，**只需重启一次虚拟机**，系统在引导时会自动将根分区和文件系统撑满整个硬盘，无需任何手动干预。

**手动热扩容（不重启）**：
如果您在虚拟机运行期间动态增加了硬盘容量，且不想重启系统，请在系统内手动执行以下两行命令进行热扩容（假设根磁盘为 `vda`）：

```bash
growpart /dev/vda 1   # 扩展分区（如果是 NVMe 磁盘，则为 /dev/nvme0n1 1）
resize2fs /dev/vda1   # 扩展文件系统（如果是 NVMe 磁盘，则为 /dev/nvme0n1p1）
```